...
image_channels = dataset_cfg["im_channels"]  # Use latent channels
...
model = Unet(im_channels=dataset_cfg['im_channels'], model_config=ldm_cfg).to(device)
# print(f"(image_size:{image_size}, image_channels:{image_channels}, batch_size:{batch_size}, num_epochs:{num_epochs}, lr:{lr})")
...
#optimizer = AdamW8bit(model.parameters(), lr=train_cfg['lr'])
...
train_dataset = PTTFaceDataset(
    root_dir=dataset_cfg['im_path'],
    image_size=dataset_cfg['im_size'],
    split='train'
)
valid_dataset = PTTFaceDataset(
    root_dir=dataset_cfg['im_path'],
    image_size=dataset_cfg['im_size'],
    split='valid'
)
...
best_loss = float('inf')
checkpoint_dir = "./checkpoints_pixel"
os.makedirs(checkpoint_dir, exist_ok=True)

grad_accum_steps = 4  # ✅ 每幾步累積一次再更新

from torch.amp import autocast  # ✅ 用新寫法
best_loss = float('inf')
checkpoint_dir = "./checkpoints_pixel"
os.makedirs(checkpoint_dir, exist_ok=True)
grad_accum_steps = 4
...
for epoch in range(num_epochs):
    ...
    for step, imgs in enumerate(pbar):
        imgs = imgs.to(device)
        t = torch.randint(0, noise_scheduler.num_timesteps, (imgs.size(0),), device=device)
        noise = torch.randn_like(imgs)
        noisy_imgs = noise_scheduler.add_noise(imgs, noise, t)

        with autocast("cuda"):
            noise_pred = model(noisy_imgs, t)
            loss = loss_fn(noise_pred, noise) / grad_accum_steps
        ...
        pbar.set_description(f"Train Loss: {loss.item() * grad_accum_steps:.4f}")
    ...
    if (epoch + 1) % 100 == 0:
        torch.save({
            ...
            'loss': avg_loss,
            'scheduler_state_dict': scheduler.state_dict(),
        }, os.path.join(checkpoint_dir, f"checkpoint_epoch_{epoch+1:03d}.pt"))
    ...
    if (epoch) % 10 == 0:
        with torch.no_grad():
            model.eval()
            x = torch.randn(4, image_channels, image_size, image_size).to(device)
            ...
            model.train()
            save_image(x.clamp(-1, 1).add(1).div(2), f"{save_dir}/sample_epoch_{epoch+1:03d}.png")
